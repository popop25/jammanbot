...
잠만봇은 Slack DM처럼 대화하면서 구내식당 메뉴 확인, 식사 기록, 음식 메뉴 추천을 돕는 LunchLog Agent입니다. 기존 Slack 요약/링크 요약 기능은 legacy 기능으로 남아 있습니다.
...
- 웹에서 잠만봇과 1:1 DM처럼 대화
- `오늘 점심 뭐야?`처럼 물으면 구내식당 메뉴와 이미지를 채팅 메시지로 응답
- `A코너 먹었고 맛있었어`처럼 말하면 메뉴명과 만족도 기록
- `이번 주 뭐 먹었지?`로 식사 패턴 요약
- `오늘 뭐 먹을까?`로 음식 메뉴 룰렛
- `식당 비원으로 바꿔줘`처럼 기본 식당 변경
- legacy Slack 기능:
- `@잠만봇 뭐 얘기했어?`
- `@잠만봇 요약`
- `@잠만봇 못 본 거 요약`
